import { NextResponse } from 'next/server';
import { getAdminDb } from '@/lib/firebase/admin';
import { FieldValue } from 'firebase-admin/firestore';

export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
    const { id } = await params;
    try {
        const { password } = await request.json();
        if (!password) {
            return NextResponse.json({ error: 'Password is required' }, { status: 400 });
        }

        const db = getAdminDb();
        const snap = await db.collection('notes').doc(id).get();

        if (!snap.exists) {
            return NextResponse.json({ error: 'Note not found' }, { status: 404 });
        }
        const note = snap.data();

        if (note?.expiresAt) {
            const exp = note.expiresAt.toDate();
            if (exp < new Date()) {
                return NextResponse.json({ error: 'This note has expired' }, { status: 410 });
            }
        }

        const { decrypt } = await import('@/lib/encryption');

        // 1. Decrypt and verify password
        let decryptedPassword = '';
        try {
            decryptedPassword = decrypt(note?.password || '');
        } catch (err) {
            // If decryption fails, the note is either malformed or legacy (which we no longer support)
            return NextResponse.json({ error: 'Incorrect password' }, { status: 401 });
        }

        if (decryptedPassword !== password) {
            return NextResponse.json({ error: 'Incorrect password' }, { status: 401 });
        }

        // 2. Decrypt content
        let decryptedContent = '';
        try {
            decryptedContent = decrypt(note?.content || '');
        } catch (err) {
            return NextResponse.json({ error: 'Could not decrypt note content' }, { status: 500 });
        }

        await db.collection('notes').doc(id).update({ viewCount: FieldValue.increment(1) });
        return NextResponse.json({ content: decryptedContent }, { status: 200 });
    } catch (err) {
        console.error('POST /api/notes/[id]/unlock error:', err);
        return NextResponse.json({ error: 'Something went wrong.' }, { status: 500 });
    }
}
